    ...
    <div class="flex items-center justify-between">
        ...
        <div class=" flex items-center">
            ...
            <button type="{{ $type }}" @if ($wireClick) wire:click="{{ $wireClick }}" @endif
                ...
                class="bg-accent/20 hover:bg-accent/30 text-accent px-5 py-2 rounded-full text-sm font-medium transition-all border border-accent/20 disabled:opacity-50 disabled:cursor-not-allowed">
                {{ $name }}
            </button>
        </div>
    </div>
